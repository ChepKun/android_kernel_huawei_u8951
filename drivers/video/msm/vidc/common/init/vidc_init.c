				...
				enum buffer_dir buffer)
{
	...
	for (i = 0; i < *num_of_buffers; ++i) {
  	...
  	len = sizeof(client_ctx->recon_buffer)/sizeof(struct vcd_property_enc_recon_buffer);
  	for (i = 0; i < len; i++) {
    		if (!vcd_get_ion_status()) {
      			...
      			if (!IS_ERR_OR_NULL(client_ctx->recon_buffer_ion_handle[i])) {
        				ion_unmap_kernel(client_ctx->user_ion_client, client_ctx->recon_buffer_ion_handle[i]);
        				if (!res_trk_check_for_sec_session() && (res_trk_get_core_type() != (u32)VCD_CORE_720P)) {
